...
udp_api_handle_t udp_api_create(uint16_t localport, const char* remoteip, uint16_t remoteport, const char* name){
	...
	tmplen = udp_snprintf(tmp, 6, "%s", name);
	udp_snprintf(tmp+tmplen,16 - tmplen, "_r");
	handler->eth_recv_handle = udp_thread_create(tmp, udp_recv_loop, handler->eth_handle);
	udp_snprintf(tmp+tmplen,16 - tmplen, "_s");
	handler->eth_send_handle = udp_thread_create(tmp, udp_send_loop, handler->eth_handle);
...
void  udp_api_send(udp_api_handle_t* phandler, uint8_t* data, uint32_t datalen){
	udp_api_handler_t* handler = (udp_api_handler_t*)phandler;
	udp_eth_send(handler->eth_handle, data, datalen);
}
void  udp_api_recv(udp_api_handle_t* phandler, uint8_t* data, uint32_t datalen){
	udp_api_handler_t* handler = (udp_api_handler_t*)phandler;
	udp_eth_recv(handler->eth_handle, data, datalen);
}