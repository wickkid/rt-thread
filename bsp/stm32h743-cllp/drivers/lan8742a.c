...
#if defined(RT_USING_LWIP) && defined(RT_USING_EMAC)
...
/** @defgroup LAN8742_Private_Defines LAN8742 Private Defines
  * @{
  */
#define LAN8742_SW_RESET_TO    ((uint32_t)(5*RT_TICK_PER_SECOND))	//software reset timeout deadline
#define LAN8742_INIT_TO        ((uint32_t)(2*RT_TICK_PER_SECOND))//Wait for 2000ms to perform initialization  2000U
...
static int32_t LAN8742_Init(lan8742_Object_t *pObj)
{
	...
	{
		tickstart =  pObj->IO.GetTick();
		rt_kprintf("LAN8742A wait for initiializing... ...\n");
		/* Wait for 2s to perform initialization */
		while((pObj->IO.GetTick() - tickstart) <= LAN8742_INIT_TO)
		{
		}
		rt_kprintf("LAN8742A initiialized\n");
...
static int32_t LAN8742_GetLinkState(lan8742_Object_t *pObj)
{
	uint32_t readval = 0;
  /*{
		rt_tick_t d = rt_tick_get();
		while ((rt_tick_get() - d) < (RT_TICK_PER_SECOND*2)) __nop();
	}*/
...
static rt_err_t rt_lan8742_init(rt_device_t dev)
{
	...
	{
		eth_device_linkchange(&lan8742_device.parent, RT_FALSE);

	}
	else{
		rt_kprintf("link up\n");
		eth_device_linkchange(&lan8742_device.parent, RT_TRUE);
	}
	return RT_EOK;
}
	...
	if(HAL_ETH_GetRxDataBuffer(&EthHandle, &RxBuff) == HAL_OK) 
	{
		HAL_ETH_GetRxDataLength(&EthHandle, &framelength);
		  ...
		  p = pbuf_alloc(PBUF_LINK, framelength, PBUF_RAM);
			...
			{
				/* copy to the pbuf */
					//pbuf_take(p, (void*)(desc->buffer1 | 0xa0000000), length);
					//rt_hw_dcache_invalidate_range((rt_ubase_t)(desc->buffer1), length);
					//pbuf_take(p, (void*)(desc->buffer1), length);
